...
begin
  ...
  PChar('Code by hackbard'+#10#13+
    ''+#10#13+
    ''+#10#13+
    'The Component for the UnRar Library for Delphi written by Philippe Wechsler'+#10#13+
    ''+#10#13+
    'Source for this demo and the UnRar Component for free.'+#10#13+
...
procedure TForm1.Archiveinformation1Click(Sender: TObject);
begin
  Application.MessageBox(
...
procedure TForm1.ExtractSelected1Click(Sender: TObject);
var
  Path:String;
  i:integer;
  files:TStrings;
...
procedure TForm1.RARArchiveError(Sender: TObject; const ErrorCode: Integer;
  const Operation: TRAROperation);
var
  op:String;
begin
  ...
  Memo1.lines.add(op+': '+inttostr(ErrorCode));
end;

procedure TForm1.RARArchiveListFile(Sender: TObject;
  const FileInformation: TRARFileItem);
var
  Attrib:String;
begin
...
procedure TForm1.RARArchiveNextVolumeRequired(Sender: TObject;
...
procedure TForm1.RARArchivePasswordRequired(Sender: TObject;
  const HeaderPassword: Boolean; const FileName: string;
  out NewPassword: string; out Cancel: Boolean);
begin
...
procedure TForm1.RARArchiveProgress(Sender: TObject; const FileName: WideString;
  const ArchiveBytesTotal, ArchiveBytesDone, FileBytesTotal,
  FileBytesDone: Cardinal);
begin
...
procedure TForm1.RARArchiveReplace(Sender: TObject; const ExistingData,
  NewData: TRARReplaceData; out Action: TRARReplace);
var
